...
export function Section({
  ...
  const renderHeader =
    typeof header === "function"
      ? (header as SectionHeaderRenderer)(headingLevel)
      : header;

  return (
    <SectionDepthContext.Provider
      value={{ depth: nextDepth, baseLevel: effectiveBase }}
    >
      <Box
        component={Component}
        className={className}
      ...
      >
        {renderHeader ? (
          <Box style={{ marginBottom: headerGap ?? "var(--ds-space-sm)" }}>
            {renderHeader}
          </Box>
        ) : null}
        {subheader ? (
          <Box style={{ marginBottom: subheaderGap ?? "var(--ds-space-sm)" }}>
            {subheader}
          </Box>
        ) : null}
        <Box>{children}</Box>
